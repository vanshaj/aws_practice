...
vpc_id = `aws ec2 create-vpc \
	...
	--instance-tenancy default | jq '.Vpc.VpcId'`
...
subnet_id=`aws ec2 create-subnet \
	...
	--cidr-block 172.32.1.0/24 | jq '.Subnet.SubnetId'`
echo "Created new subnet with id $subnet_id"

# Create an Internet Gateway
ig_id=`aws ec2 create-internet-gateway \
	--tag-specification '[{"ResourceType": "internet-gateway", "Tags": [{"Key": "Name", "Value": "IG-1"}]}]' | jq '.InternetGateway.InternetGatewayId'`
...
route_table_id=`aws ec2 create-route-table \
		--vpc-id $vpc_id | jq '.RouteTable.RouteTableId'`
...
sg_id=`aws ec2 create-security-group \
	...
	--vpc-id $vpc_id \
	--description "Created Persnal SG" | jq '.GroupId'`
...
instance_id=`aws ec2 run-instances \
	...
	--block-device-mappings '[{"DeviceName":"/dev/xvda","Ebs":{"VolumeSize":20,"VolumeType":"gp2"}}]' | jq '.Instances[0].InstanceId'`
